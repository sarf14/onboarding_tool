...
export default function ActivityPage() {
  ...
  useEffect(() => {
    ...
    fetchActivity();
  }, [day, session, activityIndex, token, router]);

  const fetchActivity = async () => {
    try {
      const response = await api.get(`/content/activity/${day}/${session}/${activityIndex}`);
        ...
        <div style={{
          ...
          <div>
            <div style={{
              ...
              letterSpacing: '3px',
              marginBottom: '10px'
            }}>Day {day}: {activityData.sessionTitle}</div>
            <div style={{
              fontSize: '24px',
              fontWeight: 700,
              color: '#333',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                if (activityIndex > 0) {
                  router.push(`/day/${day}/activity/${session}/${activityIndex - 1}`);
                } else {
                  router.push(`/day/${day}`);
                }
              }}
